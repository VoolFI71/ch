...
def list_user_pgn_files(
    ...
    """
    Get all PGN files for lessons in courses that the user is enrolled in.
    Returns files in format: "Course Title" Урок {lesson_number}
    Only shows files from courses where the user has an enrollment.
    """
    # Get all enrollments for the user
    enrollments = db.query(Enrollment).filter(Enrollment.user_id == current_user.id).all()
    enrolled_course_ids = [e.course_id for e in enrollments]
    
    # If user has no enrollments, return empty list
    if not enrolled_course_ids:
        return []
    
    # Get all lessons with PGN content ONLY from enrolled courses
    lessons = (
        db.query(Lesson)
        .join(Course)
        .filter(
